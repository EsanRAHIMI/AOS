/**
 * Architect Agent — entry point (production bootstrap).
 *
 * All service construction lives in server.ts (buildArchitectAgentService),
...
import { buildArchitectAgentService } from './server.js';
...
async function main(): Promise<void> {
  ...
  const service = await buildArchitectAgentService(env);
