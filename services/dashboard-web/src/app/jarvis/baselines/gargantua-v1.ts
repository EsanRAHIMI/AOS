/**
 * FROZEN BASELINE — gargantua-v1
 *
 * Locked 2026-07-21: warm orbital gold + stellar outer rings, speak-driven
 * gravity (redder / orbital spin / disk thickness breath), multi-shell inward
 * gold glow with soft radial falloff on the wide horizon band.
 * Listening never drives the singularity (UI-only). Mesh shares luxPaletteFromAccent.
 *
 * Do not edit. Experiment in gargantua-v2.ts.
 *
...
export function prepareGargantuaV1(
  ctx: CanvasRenderingContext2D,
  cx: number,
  cy: number,
  radius: number,
...
export type GargantuaV1PartId = keyof typeof gargantuaV1Parts;

export const GARGANTUA_V1_PART_ORDER: readonly GargantuaV1PartId[] = [
  'bloom',
  'fullDisk',
  'lensWraps',
  'eventHorizon',
...
export const GARGANTUA_V1_META = {
  id: 'gargantua-v1',
  lockedAt: '2026-07-21',
  description:
    'Warm orbital gold lock — speak gravity, multi-shell inward gold glow, soft wide horizon band',
  parts: GARGANTUA_V1_PART_ORDER,
} as const;

export function drawGargantuaV1(
  ctx: CanvasRenderingContext2D,
  cx: number,
  cy: number,
  radius: number,
  ...
  const order = parts?.length
    ? GARGANTUA_V1_PART_ORDER.filter((id) => parts.includes(id))
    : GARGANTUA_V1_PART_ORDER;
  for (const id of order) gargantuaV1Parts[id](g);
}
