 ...
 *
 * Locked 2026-07-21: warm orbital gold (hot → gold → amber → orange → scarlet),
 * stellar outer rings, inward horizon limb, red photon halo, bright disk–horizon
 * contact. Mesh + singularity share `luxPaletteFromAccent`.
 *
...
export const GARGANTUA_V1_META = {
  ...
  lockedAt: '2026-07-21',
  description:
    'Warm orbital gold lock — stellar outer rings, red photon limb, bright disk–horizon contact',
...
export function drawGargantuaV1(
  ...
  const g = prepareGargantuaV1(ctx, cx, cy, radius, t, accent);
  const order = only?.length
    ? GARGANTUA_V1_PART_ORDER.filter((id) => only.includes(id))
