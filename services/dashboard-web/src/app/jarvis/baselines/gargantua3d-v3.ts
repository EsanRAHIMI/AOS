/**
 * FROZEN — gargantua3d-v3  (locked 2026-07-25)
 *
 * Byte-exact snapshot of the owner's upgraded Gargantua (forked from the live
 * gargantua3d-v2 at the moment the /jarvis stage was visually overhauled).
 * This is the RESTORE POINT for that look — do NOT edit this file.
 *
 * Restore cue: «برگردون به gargantua3d-v3» → in ../gargantua3d.ts re-export
 *   createGargantua3DV3 as createGargantua3D (+ GARGANTUA_LOCK_V3 as
 *   GARGANTUA_LOCK). Nothing else needs to change: the module contract
 *   (Gargantua3D type) is identical across all baselines.
 *
 * Lineage: gargantua3d-v1 (frozen) → v2 (live/dev) → v3 (this snapshot).
 */
...
export const GARGANTUA3D_V3_META = {
  id: 'gargantua3d-v3',
...
export const GARGANTUA_LOCK_V3: { theta: number; phi: number; dist: number } = {
  theta: 0.28,
  phi: 1.48,
  dist: 6.25,
};
...
export function createGargantua3DV3(canvas: HTMLCanvasElement): Gargantua3D {
  ...
  const uniforms = {
    ...
    uRadiusPx: { value: 80 },
    uCamTheta: { value: GARGANTUA_LOCK_V3.theta },
    uCamPhi: { value: GARGANTUA_LOCK_V3.phi },
    uCamDist: { value: GARGANTUA_LOCK_V3.dist },
  };
  ...
  const orbit = {
    theta: GARGANTUA_LOCK_V3.theta,
    phi: GARGANTUA_LOCK_V3.phi,
    dist: GARGANTUA_LOCK_V3.dist,
    dragging: false,
    lastX: 0,
    lastY: 0,
    pointerId: -1,
  ...
  const onDbl = (e: Event) => {
    e.preventDefault();
    orbit.theta = GARGANTUA_LOCK_V3.theta;
    orbit.phi = GARGANTUA_LOCK_V3.phi;
    orbit.dist = GARGANTUA_LOCK_V3.dist;
  };
