/**
 * DEVELOPMENT — gargantua3d-v2
 *
 * Forked from frozen gargantua3d-v1 (2026-07-22). Apply further look / motion /
 * orbit experiments here. To restore the locked baseline, point gargantua3d.ts
 * at createGargantua3DV1.
 */
...
export const GARGANTUA3D_V2_META = {
  id: 'gargantua3d-v2',
  forkedFrom: 'gargantua3d-v1',
  label: 'Cinematic raymarch (dev)',
} as const;

/** Slight ¾ perspective — not face-on, not razor edge-on */
export const GARGANTUA_LOCK_V2: { theta: number; phi: number; dist: number } = {
  theta: 0.28,
  phi: 1.48,
  dist: 6.25,
};
...
export function createGargantua3DV2(canvas: HTMLCanvasElement): Gargantua3D {
  ...
  const uniforms = {
    ...
    uCamDist: { value: GARGANTUA_LOCK_V2.dist },
  };
  ...
  const orbit = {
    theta: GARGANTUA_LOCK_V2.theta,
    phi: GARGANTUA_LOCK_V2.phi,
    dist: GARGANTUA_LOCK_V2.dist,
    dragging: false,
    lastX: 0,
    lastY: 0,
    pointerId: -1,
  ...
  const onDbl = (e: Event) => {
    e.preventDefault();
    orbit.theta = GARGANTUA_LOCK_V2.theta;
    orbit.phi = GARGANTUA_LOCK_V2.phi;
    orbit.dist = GARGANTUA_LOCK_V2.dist;
  };
