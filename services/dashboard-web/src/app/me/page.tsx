  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={`Scope: personal (${ctx?.tenant?.name ?? ''}) · data freshness: ${reality?.graph.dataFreshness?.slice(0, 10) ?? 'no data yet'} · ${ctx?.activeConsents ?? 0} connector consent(s)`}
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        {acts.length === 0 ? (
          <EmptyState icon="·" title="Nothing to rank yet" hint="Ingest your baseline first: profile, goals, projects, risks via POST /v1/me/reality/ingest — or tell the operator “build my personal reality baseline”." />
        ) : (
