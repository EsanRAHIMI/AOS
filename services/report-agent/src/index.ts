/**
 * Report Agent — entry point (production bootstrap).
 *
 * All service construction lives in server.ts (buildReportAgentService),
...
import { buildReportAgentService } from './server.js';
...
async function main(): Promise<void> {
  ...
  const service = await buildReportAgentService(env);
