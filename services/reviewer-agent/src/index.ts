/**
 * Reviewer Agent — entry point (production bootstrap).
 *
 * All service construction lives in server.ts (buildReviewerAgentService),
...
import { buildReviewerAgentService } from './server.js';
...
async function main(): Promise<void> {
  ...
  const service = await buildReviewerAgentService(env);
