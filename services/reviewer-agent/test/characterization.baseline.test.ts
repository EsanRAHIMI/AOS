...
import { buildReviewerAgentService, manifest } from '../src/server.js';
...
async function buildTestApp() {
  ...
  setTestDb(db);
  const service = await buildReviewerAgentService({
...
describe('reviewer-agent baseline characterization', () => {
  ...
  it('GET /health returns ok + this service id', async () => {
    ...
    expect(res.json()).toEqual({ status: 'ok', serviceId: 'reviewer-agent' });
  });
  ...
  it('GET /.factory/manifest returns this service\'s static manifest', async () => {
    ...
    expect(res.statusCode).toBe(200);
    const body = res.json();
    expect(body.data.serviceId).toBe('reviewer-agent');
    expect(body.data.domain).toContain('reviewer');
  });
  ...
  it('GET /.factory/status returns 200 with this serviceId', async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect(res.json().data.serviceId).toBe('reviewer-agent');
  });
  ...
  it('POST /.factory/task with no token -> 401 error envelope', async () => {
    const { app } = await buildTestApp();
    const res = await app.inject({ method: 'POST', url: '/.factory/task', payload: { goal: 'review the plan' } });
  ...
  it('POST /.factory/task (forceFallback) writes traces/cost/evidence/review_reports/agent_runs and publishes REVIEW_COMPLETED', async () => {
    ...
    expect(body.data.review.mode).toBe('fallback');
    ...
    expect(writes('review_reports')).toHaveLength(1);
    expect(writes('agent_runs')).toHaveLength(1);

    const publishedTypes = publishSpy.mock.calls.map((c) => (c[0] as { type: string }).type);
    expect(publishedTypes).toEqual(['agent.run.started', 'intel.review.completed']);
  });
});
