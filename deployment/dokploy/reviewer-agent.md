# Dokploy — reviewer-agent

Independent Dokploy application (no Docker locally; Dokploy builds from GitHub).

| Setting | Value |
|---|---|
| App name | reviewer-agent |
| Domain | https://reviewer.simorx.com |
| Repository | github.com/<owner>/autonomous-os-kernel |
| Root directory | services/reviewer-agent |
| Build command | pnpm install --frozen-lockfile && pnpm --filter @factory/reviewer-agent... build |
| Start command | node services/reviewer-agent/dist/index.js |
| Health check | /health |
| Internal port | 4106 |
