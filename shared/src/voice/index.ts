...
/** Phase 19.5 — normalized command identity (dedupe key), shared by gateway + tests.
 *  The dashboard client keeps an identical copy in its dependency-free gate. */
...
export function routeUtterance(text: string, ctx: VoiceContextLite): ToolProposal {
  ...
  // --- Read / explain (the gateway composes the specific live-state answer) ---
  if (/(what.*happening|what's going on|current status|^status\b|explain (this|the)? ?page|where am i|summari[sz]e)/.test(t)) {
    return readTool('explain_current_page', 'Reading live system state.');
  }
  if (/(next (best )?action|what should i do|what.*next)/.test(t)) {
    return readTool('read_status', 'Reading live system state for the next best action.');
  }
  if (/(pending )?approvals?|what.*approve/.test(t)) {
    return readTool('list_pending_approvals', 'Listing approvals waiting for a decision.');
  }
  if (/(show|open) (the )?report/.test(t)) return readTool('open_report', 'Opening the latest intelligence report.');
  if (/(show|list) evidence|what proof/.test(t)) return readTool('show_evidence', 'Showing the most recent evidence records.');

  // --- Whole-system read-only check ("check the system", "system status check") ---
  if (/(check|verify|inspect)\b.*(system|everything|all services)|system (check|status|health)|full (check|status)/.test(t)) {
    return {
      toolName: 'run_system_status_check', category: 'low_mutation', args: {},
      riskLevel: 'low', requiresApproval: false, ownerOnly: false, blocked: false, blockedReason: '', confirm: 'light',
      explanation: 'I’ll check live services, approvals, incidents, Dokploy sync, and readiness. This is read-only. Confirm?',
      guardrailNote: 'System check is read-only aggregation — never a Dokploy operation.',
    };
  }
  ...
  if (/\b(health|healthy|reachable|is .* up|check .*(service|gateway|api|agent))\b/.test(t) || (/check/.test(t) && svc)) {
    return {
      ...
      riskLevel: 'low', requiresApproval: false, ownerOnly: false, blocked: false, blockedReason: '', confirm: 'light',
      explanation: `Read-only health check${svc ? ` on ${svc}` : ''}: real /health + registry verification, evidence stored. No changes. Confirm?`,
  ...
  // --- Fallback: echo what was heard, one short line, no capability spam ---
  const heard = text.trim().slice(0, 80);
  return readTool('unrecognized_command', `I heard: “${heard}”. I can’t map that to a safe operation. Try “check the system”, “check gateway health”, “analyze history”, “security check”, or “research <topic>”.`);
}
