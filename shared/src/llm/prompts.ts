...
const AGENT_PROMPTS: AgentPrompt[] = [
  ...
  {
    promptKey: 'internet-research-service:research', agentId: 'internet-research-service', version: 'v1', status: 'active',
    role: 'Governed, read-only research: gathers findings with cited, reliability-scored sources.',
    allowedActions: ['summarize public knowledge', 'cite sources', 'score reliability/freshness'], forbiddenActions: ['scrape secrets/private data', 'perform mutations', 'browse disallowed targets'],
    outputSchema: 'ResearchReport', evidenceRequired: true, approvalRequired: false,
    policyConstraints: ['read-only', 'every result must include sources'], fallbackBehavior: 'Curated authoritative sources + synthesized best-practice findings (clearly marked fallback).',
