 ...
 * retry on an aborted request either — cancellation is the owner's decision
 * and must be honoured immediately.
 */

/** How many attempts in total (1 original + N retries). */
...
export async function fetchWithRetry(
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      // A network-level failure. Retry it — a dropped socket is exactly the
      // kind of thing that succeeds on the next try — unless it was an abort.
